...
public class BlockchainServiceImpl implements IBlockchainService {
    ...
    @Override
    public Map<String, JSONArray> queryActBlock(String blockNum) {
        log.info("BlockchainServiceImpl|queryActBlock 开始处理【blockNum={}】", blockNum);
        JSONArray jsonArray = new JSONArray();
        Map<String, JSONArray> transactions = new HashMap<>(1);
        try {
            jsonArray.add(blockNum);
            ...
            transactions.put(createTaskJson.getString("id"), createTaskJson.getJSONArray("user_transaction_ids"));
        } catch (Exception er) {
    ...
    @Override
    public List<ActTransaction> queryTransactions(Map<String, JSONArray> map) {
        log.info("BlockchainServiceImpl|saveTransactions 开始处理[map={}]", map);
        List<ActTransaction> actTransactions = new ArrayList<>();
        map.keySet().forEach(s -> map.get(s).forEach(j -> {
...
//                return null;
//            }
            if (!config.actAddresses.contains(actTransaction.getFromAddr()) &&
                    !config.actAddresses.contains(actTransaction.getToAddr())){
                return null;
            }
    ...
    private JSONObject getEvent(String blockId, String trxId, ActTransaction actTransaction) {
        ...
        jsonArrayEvent.add(actTransaction.getBlockNum());
